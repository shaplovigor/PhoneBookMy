...
public class PhoneBook {
    ...
    public static void main(String[] args) {
        ...
        // идем циклом по справочнику с целью его заплонения
        exitLabel: for (int j=0; j < PhoneBook.length; j++ ) {

                Scanner console = new Scanner(System.in);
                String FIO = "";
                String phoneNumber = "";
                System.out.println("Введите ФИО (для выхода введите Exit):");
                boolean isCorrectName = false;
                while (!isCorrectName) {
                    // считываем новую строку с консоли: ФИО, форматируем и проверяем корректность
                    FIO = formatName(console.nextLine()).trim();
                    if (FIO.equals("Exit") || FIO.equals("exit")) break exitLabel; // выход по Exit
                    isCorrectName = checkName(FIO);
                    if (!isCorrectName) System.out.println("Введите корректное имя!");
                }

                // перебираем построчно справочник и ищем введенное ФИО
                boolean isFind = false;
                for (int i = 0; i < PhoneBook.length; i++)
                // если ФИО есть - возвращаем номер телефона
                if (FIO.equals(PhoneBook[i][0])) {
                    isFind = true;
                    System.out.println(FIO + " уже есть в справочнике, телефон: " + PhoneBook[i][1]);
                    break;
                }
                // если ФИО нет - запрашиваем номер телефона и добавляем запись ФИО + номер
                if (!isFind) {
        ...
        list(PhoneBook); // печать справочника
